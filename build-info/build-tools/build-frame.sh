...
function add(){
        ...
        echo  "yt-dlp -f 'ba' -x --audio-format mp3 $link -o '$dir/%(playlist_index)02d - %(title)s.%(ext)s' --download-archive ../build-custom/$list/build-logs/$name.ark " ">"">" "../build-custom/$list/build-logs/$name/""$""date""/download.log" >> ../build-custom/$list/build-list/$name
